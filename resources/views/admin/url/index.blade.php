                        ...
                        @if(!empty($urls ))
                            @foreach($urls as $key=> $val)
                                <tr class="item{{$val->id}}">
                                    ...
                                    @include('admin.url.edit',['key'=>$key])

                                    <td  class="text-center  text-light ">
                                        <a href="#" data-id="{{ $val->id }}" data-route="{{ route('admin.url.destroy',$val->id) }}"  type="submit" style="width: max-content;" class="btn-sm btn-danger btnDelete" ><i style="margin: 0 0 0 5px;" class="fa fa-trash"></i><i style="margin: inherit;">حذف</i></a>
                                    </td>
                                </tr>
                            @endforeach
                        @endif
