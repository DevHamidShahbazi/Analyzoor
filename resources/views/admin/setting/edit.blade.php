...
{{--                    </div>--}}
                    ...
                    <div class="form-group">
                        <label class="col-sm-12 control-label">{{setting_with_key('courses_home')->label}}</label>
                        <div class="col-sm-6">
                            <select name="courses_home[]" class="form-control select2"  multiple="multiple"
                                    data-placeholder="{{setting_with_key('courses_home')->label}}"
                                    style="width: 100%;text-align: right">
                                @php $courses_home = \App\Models\Article::where('is_active','1')->get() @endphp
                                @php $courses_home_specify = setting_with_key('courses_home')->value @endphp
                                @foreach ($courses_home as $course_home)

                                    <option {{ $courses_home_specify ? in_array($course_home->id ,$courses_home_specify ) ? 'selected' : ' ': ' '  }} value="{{ $course_home->id }}">
                                        {{ $course_home->name }}</option>
