    ...
    <form class="form" method="GET" action="{{ \Illuminate\Support\Facades\URL::current() }}">
        ...
        <div class="col-lg-2">
            ...
            <select class="form-control filter" name="status">
                <option @if(isset($checkbox))  {{ $checkbox['status'] == '0' ? 'selected' : ' '  }}  @endif value="0">همه</option>
                @foreach(config('fanoram.userLevel') as $persian_name => $real_name)
                    <option @if(isset($checkbox))  {{ $checkbox['status'] == $real_name ? 'selected' : ' ' }}  @endif  value="{{$real_name}}">{{$persian_name}}</option>
                @endforeach
            </select>
        </div>
